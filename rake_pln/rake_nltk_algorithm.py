...
class RakeKeywordExtractor:
  ...
  def __init__(self):
    ...
    self.stopwords = set(nltk.corpus.stopwords.words(self.language))  # stopwords.words('portuguese')
    #archive.close
  ...
  def _generate_candidate_keywords(self, sentences):
    ...
    for sentence in sentences:
      words = map(lambda x: "|" if x in self.stopwords else x,
        nltk.word_tokenize(sentence.lower(), language=self.language))
      phrase = []
      for word in words:
  ...
  def extract(self, text, incl_scores=False):
    sentences = nltk.sent_tokenize(self._check_final_simbol(text), language=self.language)  # nltk.data.load('tokenizers/punkt/portuguese.pickle')
    ...
    if incl_scores:
      sps = sorted_phrase_scores[0:int(n_phrases / self.top_fraction)]
      #s = self._define_keywords(sps)
      return sps
    else:
      sps = map(lambda x: x[0], sorted_phrase_scores[0:int(n_phrases / self.top_fraction)])
      #sps = self._define_keywords(sps)
      return sps
  ...
  def _define_keywords(self, text):
    keywords = []
    for t in text:
        if len(nltk.word_tokenize(t[0], self.language)) == 1:
            keywords.append(t[0])
        elif len(nltk.word_tokenize(t[0], self.language)) == 2:
            words = test_extract_parsing_1(t[0])
            for w in words:
                keywords.append(w)
    print (keywords)
    return keywords
...
def test1():
  ...
  text = "Que horas será o evento FooFizzBuzz?"
  r = rake.extract(text, incl_scores=True)
  print (r)
  
  '''
  keywords = []
  for t in r:
      if len(nltk.word_tokenize(t[0], language)) == 1:
          keywords.append(t[0])
      elif len(nltk.word_tokenize(t[0], language)) == 2:
          words = test_extract_parsing_1(t[0])
          for w in words:
              keywords.append(w)
  print (keywords)
